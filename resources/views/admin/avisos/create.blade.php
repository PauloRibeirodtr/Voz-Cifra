...
@section('content')
    <div class="admin-page-intro">
        <p class="admin-page-kicker">Comunicação</p>
        <h1 class="admin-page-title mt-2 text-2xl font-bold">Enviar aviso</h1>
        <p class="admin-page-copy mt-3 text-sm sm:text-base">
            Envie uma mensagem simples para todos, uma igreja, um papel operacional ou um usuário específico.
        </p>
    </div>
            ...
            <div class="grid grid-cols-1 gap-5">
                <div>
                    ...
                    <select id="escopo" name="escopo" class="mt-1 block w-full rounded-xl border border-gray-300 bg-white px-4 py-3 text-gray-800 focus:border-green-600 focus:ring-2 focus:ring-green-100" data-aviso-escopo>
                        <option value="todos" @selected(old('escopo', 'todos') === 'todos')>Todos os usuários ativos</option>
                        ...
                        <option value="usuario" @selected(old('escopo') === 'usuario')>Um usuário específico</option>
                    </select>
                </div>
                ...
                <div class="grid grid-cols-1 gap-5 md:grid-cols-3">
                    ...
                    <div data-aviso-campo="usuario">
                        ...
                        <label for="usuario_busca" class="block text-sm font-semibold text-gray-700">Usuário</label>
                        ...
                        <p class="mt-2 text-xs text-gray-500" data-aviso-usuario-ajuda>
                            Digite 3 letras para ver sugestões e clique no usuário correto.
                        </p>
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
                    <a href="{{ route('admin.settings') }}" class="inline-flex items-center justify-center rounded-xl border border-gray-200 bg-white px-5 py-3 font-semibold text-gray-700 hover:bg-gray-50">
                        Cancelar
                    </a>
...
@push('scripts')
    @php
        $usuariosAviso = $usuarios->map(fn ($usuario) => [
            'id' => $usuario->id,
            'nome' => $usuario->nome,
            'email' => $usuario->email,
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const usuarios = @json($usuariosAviso, JSON_UNESCAPED_UNICODE);
            ...
            const renderizarUsuarios = () => {
                ...
                if (encontrados.length === 0) {
                    usuarioResultados.innerHTML = '<div class="px-4 py-3 text-sm text-gray-500">Nenhum usuário encontrado.</div>';
