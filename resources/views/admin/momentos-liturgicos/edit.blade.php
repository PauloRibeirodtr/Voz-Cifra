...
@section('title', 'Editar momento liturgico | Voz & Cifra')
@section('mobile_title', 'Editar momento')
...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Editar momento liturgico</h1>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            <div class="grid grid-cols-1 gap-4">
                <div>
                    ...
                    <input type="text" name="nome" value="{{ old('nome', $momentoLiturgico->nome) }}" required placeholder="Ex.: Entrada" class="{{ $classeInput }}" />
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700">Descricao</label>
                    <textarea name="descricao" rows="5" placeholder="Descreva brevemente este momento liturgico." class="{{ $classeInput }}">{{ old('descricao', $momentoLiturgico->descricao) }}</textarea>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700">Ordem de exibicao</label>
                    <input type="number" name="ordem_exibicao" value="{{ old('ordem_exibicao', $momentoLiturgico->ordem_exibicao) }}" min="1" placeholder="Ex.: 1" class="{{ $classeInput }}" />
                </div>

                <div class="flex items-center gap-3 pt-2">
                    <input type="hidden" name="ativo" value="0" />
                    <input id="ativo" type="checkbox" name="ativo" value="1" {{ old('ativo', $momentoLiturgico->ativo) ? 'checked' : '' }} class="rounded border-gray-300 text-green-700 focus:ring-green-500" />
