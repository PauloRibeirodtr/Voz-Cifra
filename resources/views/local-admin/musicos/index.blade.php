...
@section('title', 'Músicos da igreja | Voz & Cifra')
@section('mobile_title', 'Músicos')

@section('content')
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Músicos da igreja</h1>
            <p class="mt-1 text-sm text-gray-500">Cadastre e organize apenas os músicos vinculados à {{ $igreja->nome }}.</p>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        @if ($musicos->isEmpty())
            <div class="p-8 text-center text-gray-500">Nenhum músico cadastrado para esta igreja.</div>
        @else
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="inline-flex flex-wrap items-center justify-end gap-2">
                                        <a href="{{ route('local-admin.musicos.edit', $musico) }}" class="inline-flex rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Editar</a>
                                        <form action="{{ route('local-admin.musicos.password.reset', $musico) }}" method="POST" onsubmit="return confirm('Deseja resetar a senha deste músico para o CPF e exigir troca no próximo acesso?');">
                                            @csrf
                                        ...
                                        <form action="{{ route('local-admin.musicos.toggle', $musico) }}" method="POST" onsubmit="return confirm('Deseja alterar o status deste músico?');">
                                            @csrf
                                        ...
                                        <form action="{{ route('local-admin.musicos.destroy', $musico) }}" method="POST" onsubmit="return confirm('Deseja excluir este músico?');">
                                            @csrf
                                            @method('DELETE')
                    ...
                    <article class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                ...
                                @if ($musico->telefone)
                                    <p class="mt-1 text-xs text-gray-400">{{ $musico->telefone }}</p>
                                @endif
                            </div>
                        ...
                        <div class="mt-4 grid grid-cols-1 gap-2">
                            <a href="{{ route('local-admin.musicos.edit', $musico) }}" class="inline-flex items-center justify-center rounded-xl border border-gray-200 px-4 py-3 text-sm font-semibold text-gray-700 hover:bg-gray-50">Editar</a>
                            <form action="{{ route('local-admin.musicos.password.reset', $musico) }}" method="POST" onsubmit="return confirm('Deseja resetar a senha deste músico para o CPF e exigir troca no próximo acesso?');">
                                @csrf
